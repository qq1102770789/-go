...
import (
	...
	"gorm.io/gorm"
	"mxshop_sevs/inventory_srv/global"
	"mxshop_sevs/inventory_srv/model"
	"mxshop_sevs/inventory_srv/model/proto"
	"sync"
)
...
func (*InventoryServer) SetInv(ctx context.Context, req *proto.GoodsInvInfo) (*proto.Empty, error) {
	//	//设置库存， 如果我要更新库存
...
func (*InventoryServer) Reback(ctx context.Context, req *proto.SellInfo) (*proto.Empty, error) {
...
func (*InventoryServer) TrySell(ctx context.Context, req *proto.SellInfo) (*proto.Empty, error) {
	//扣减库存， 本地事务 [1:10,  2:5, 3: 20]
	//数据库基本的一个应用场景：数据库事务
	//并发情况之下 可能会出现超卖 1
	pool := goredis.NewPool(global.RedisClient) // or, pool := redigo.NewPool(...)
...
func (*InventoryServer) ConfirmSell(ctx context.Context, req *proto.SellInfo) (*proto.Empty, error) {
	//扣减库存， 本地事务 [1:10,  2:5, 3: 20]
	//数据库基本的一个应用场景：数据库事务
	//并发情况之下 可能会出现超卖 1
	pool := goredis.NewPool(global.RedisClient) // or, pool := redigo.NewPool(...)
...
func (*InventoryServer) CancelSell(ctx context.Context, req *proto.SellInfo) (*proto.Empty, error) {
	//扣减库存， 本地事务 [1:10,  2:5, 3: 20]
	//数据库基本的一个应用场景：数据库事务
	//并发情况之下 可能会出现超卖 1
